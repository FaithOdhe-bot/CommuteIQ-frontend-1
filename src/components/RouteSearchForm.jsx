...
export default function RouteSearchForm({ city, onCityChange, onSearch, isLoading, options, selectedMode, onSelectMode }) {
  ...
  function handleCountryClick(newCountry) {
    ...
    const firstCity = CITIES_BY_COUNTRY[newCountry][0].value;
  ...
  function handleSubmit(e) {
    ...
    onSearch({ origin: origin.trim(), destination: destination.trim(), time, city, modes: modes.map((m) => m.key) });
  }
  ...
  return (
    <form className="search-form" onSubmit={handleSubmit}>
      <div className="field">
        ...
        <div className="mode-group" role="group" aria-label="Country">
          {Object.entries(COUNTRY_META).map(([key, meta]) => (
            <button type="button" key={key} className={`mode-pill ${country === key ? "active" : ""}`}
              onClick={() => handleCountryClick(key)} aria-pressed={country === key}>
      ...
      <div className="field">
        ...
        <input id="origin" value={origin} onChange={(e) => setOrigin(e.target.value)} placeholder={placeholders.origin} required />
      </div>

      <div className="field">
        <label htmlFor="destination">To</label>
        <input id="destination" value={destination} onChange={(e) => setDestination(e.target.value)} placeholder={placeholders.destination} required />
      </div>

      <div className="field">
        <label>Mode {modesLoading && "(loading…)"}</label>
        <div className="mode-group" role="group" aria-label="Transport mode">
          {modes.map((m) => {
            const liveResult = options?.find((o) => o.mode === m.key);
            const isSelected = liveResult ? selectedMode === m.key : false;
            return (
              <button type="button" key={m.key} className={`mode-pill ${isSelected ? "active" : ""}`}
                onClick={() => liveResult && onSelectMode(m.key)}>
                {m.emoji} {m.label}
                {liveResult && ` · ${formatDuration(liveResult.travel_time_min)} ${liveResult.quality_emoji}`}
              </button>
            );
          })}
        </div>
        {modesError && <p className="form-status error">Couldn't load transport modes: {modesError}</p>}
      </div>
